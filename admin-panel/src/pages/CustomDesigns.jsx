...
const CustomDesigns = () => {
  ...
  const fetchDesignsAndProducts = async () => {
    ...
    try {
      const [designsRes, productsRes] = await Promise.all([
        axios.get('/api/designs', { withCredentials: true }),
  ...
  const handleRemoveBackground = async () => {
    ...
    try {
      ...
      // If URL mode and not a Data URL, attempt fetch Blob first to help bypass canvas CORS restrictions
  ...
  const handleAddDesignSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // 1. Fetch or create canvas printable area maps
      const printAreasRes = await axios.get(`/api/designs/print-areas/${targetProductId}`, { withCredentials: true });
      let printAreas = printAreasRes.data?.data || [];
      let printAreaId;

      if (printAreas.length === 0) {
        const newPrintAreaRes = await axios.post('/api/designs/print-areas', {
      ...
      // 2. Build customization configuration template arrays
      ...
      // 3. Dispatch data as standard JSON or Multipart FormData depending on upload strategy choice
      if (editingDesignId) {
        if (uploadMethod === 'local' && localFile) {
          ...
          await axios.patch(`/api/designs/${editingDesignId}`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
            withCredentials: true
          });
        } else {
          await axios.patch(`/api/designs/${editingDesignId}`, {
            name: newDesign.name,
      ...
      setIsAddModalOpen(false);
      setEditingDesignId(null);
      // Clean up workspace memory values
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`/api/designs/${id}`, { withCredentials: true });
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl border border-[#e2e8f0] shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-[#0f172a] tracking-tight">Custom Studio Designs</h1>
          <p className="text-sm text-[#64748b] mt-0.5">Inspect user-generated product designs, text coordinates, and layer assets.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-sm transition-all active:scale-98"
          >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredDesigns.map((d) => (
            <div 
              key={d._id} 
              className="bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col justify-between hover:shadow-md hover:border-indigo-200 transition-all"
            >
              <div className="bg-slate-50 border-b aspect-square relative flex items-center justify-center p-4">
                ...
                <div className="absolute top-3 right-3 bg-white/95 border border-slate-100 rounded-lg px-2 py-1 flex items-center gap-1 shadow-sm text-[10px] font-bold text-slate-600">
              ...
              <div className="p-4 space-y-3.5 flex-1 flex flex-col justify-between">
                <div className="space-y-1">
                  <h4 className="font-bold text-slate-800 text-sm truncate" title={d.name}>{d.name}</h4>
                ...
                <div className="flex justify-between items-center border-t border-[#f1f5f9] pt-3.5 text-[11px] text-slate-400">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 bg-indigo-50 hover:bg-indigo-100 border border-indigo-100 text-indigo-650 font-bold px-2 py-1.5 rounded-lg transition-colors shadow-sm"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 bg-amber-50 hover:bg-amber-100 border border-amber-100 text-amber-700 font-bold px-2 py-1.5 rounded-lg transition-colors shadow-sm"
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 bg-red-55 hover:bg-red-100 border border-red-100 text-red-600 font-bold px-2 py-1.5 rounded-lg transition-colors shadow-sm"
      ...
      {selectedDesign && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-2xl w-full p-6 space-y-4 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-[#f1f5f9] pb-3">
              <div className="flex items-center gap-2">
                <Eye size={18} className="text-indigo-600" />
                <h3 className="font-bold text-base text-[#0f172a]">Inspect Custom Design Layers</h3>
              </div>
              <button 
                type="button"
                onClick={() => setSelectedDesign(null)}
                className="text-slate-400 hover:text-slate-650 text-lg font-semibold"
              >
                &times;
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Design Mockup Preview</span>
                <div className="bg-slate-50 border border-slate-100 rounded-xl aspect-square flex items-center justify-center p-4 relative">
              ...
              <div className="space-y-4">
                <div className="space-y-1.5 text-xs">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Design Info</span>
                  <p className="font-bold text-slate-800 text-sm leading-tight">{selectedDesign.name}</p>
                  <p className="text-slate-500 mt-1">Creator: <span className="font-semibold text-slate-700">{selectedDesign.user?.name || 'Guest Creator'}</span></p>
                  <p className="text-slate-500">Date Added: <span className="font-semibold text-slate-700">{selectedDesign.createdAt ? new Date(selectedDesign.createdAt).toLocaleString() : 'N/A'}</span></p>
                </div>

                <div className="space-y-2">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Customization Layers ({selectedDesign.customizations?.length || 0})</span>
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {selectedDesign.customizations?.length === 0 ? (
                      <p className="text-[11px] text-slate-400 italic">No customization layers registered</p>
                    ) : (
                      selectedDesign.customizations?.map((cust, idx) => (
                        <div key={idx} className="bg-slate-50 border border-slate-200 p-3 rounded-lg space-y-3">
                          <div className="flex justify-between items-center text-[10px] font-bold text-indigo-650 uppercase border-b pb-1.5 border-slate-100">
                            <span>Area: {cust.printArea?.name || 'General'}</span>
                            <span className="flex items-center gap-1.5">
                              Canvas BG: 
                              <span 
                                className="w-3.5 h-3.5 rounded-full border border-slate-350 shrink-0" 
                          ...
                          {cust.textLayers?.length > 0 && (
                            <div className="space-y-1.5">
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Text Layers ({cust.textLayers.length})</span>
                              {cust.textLayers.map((tl, tIdx) => (
                                <div key={tIdx} className="bg-white border border-slate-150 p-2.5 rounded-lg space-y-1.5 text-xs">
                                  <p className="font-bold text-slate-800">"{tl.text}"</p>
                          ...
                          {cust.imageLayers?.length > 0 && (
                            <div className="space-y-1.5">
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">Image Layers ({cust.imageLayers.length})</span>
                              {cust.imageLayers.map((il, iIdx) => (
                                <div key={iIdx} className="bg-white border border-slate-150 p-2 rounded-lg flex gap-2 items-center text-xs">
                                  {il.url && (
                                    <img 
                                      src={il.url} 
                                      alt="Image layer" 
                                      ...
                                      className="w-10 h-10 object-contain border rounded bg-slate-50 shrink-0" 
                                    />
                                  )}
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-[#f1f5f9]">
              <button
                type="button"
                onClick={() => setSelectedDesign(null)}
                className="px-4 py-2 rounded-xl text-xs font-semibold bg-indigo-655 hover:bg-indigo-700 text-white shadow-sm transition-all"
              >
                Close Details
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-sm w-full p-6 space-y-4 animate-in fade-in zoom-in duration-200">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                <h4 className="font-bold text-slate-900 text-sm">Delete Custom Design?</h4>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <form onSubmit={handleAddDesignSubmit} className="bg-white rounded-2xl border border-slate-100 shadow-xl max-w-lg w-full p-6 space-y-4 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-[#f1f5f9] pb-3">
              <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="text-slate-400 hover:text-slate-655 text-lg font-semibold"
              >
                &times;
              </button>
            </div>
            ...
            <div className="space-y-4 text-xs">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Design Title</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl px-3 py-2 text-xs font-semibold text-slate-705 outline-none focus:border-indigo-500"
                />
              </div>

              {/* DUAL ARTWORK SELECTOR METHOD BUTTONS */}
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Design Preview Resource Strategy</label>
                <div className="grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-xl">
                  <button
                    type="button"
                    onClick={() => setUploadMethod('local')}
                    className={`flex items-center justify-center gap-1.5 py-2 text-xs font-bold rounded-lg transition-all ${uploadMethod === 'local' ? 'bg-white shadow-xs text-indigo-655' : 'text-slate-500 hover:text-slate-700'}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-1.5 py-2 text-xs font-bold rounded-lg transition-all ${uploadMethod === 'url' ? 'bg-white shadow-xs text-indigo-655' : 'text-slate-500 hover:text-slate-700'}`}
                  >
              ...
              {uploadMethod === 'local' ? (
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Choose Graphic File</label>
                  <label className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 hover:border-indigo-400 bg-slate-50/50 hover:bg-white rounded-xl py-5 cursor-pointer text-center group transition-all">
                    ...
                    <span className="text-[11px] text-slate-655 font-semibold">
                      {localFile ? `Selected: ${localFile.name}` : 'Click to browse your device directories'}
                    </span>
                    <span className="text-[9px] text-slate-400 mt-0.5">Supports PNG, JPG, JPEG, WEBP, and vector SVG files</span>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Design Preview Image URL</label>
                  <input
                    type="text"
                    ...
                    className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono text-indigo-600 outline-none focus:border-indigo-500"
                  />
                </div>
              )}
              ...
              {(uploadMethod === 'local' ? localFilePreview : newDesign.previewUrl) && (
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider">Design Asset Preview</label>
                    <span className="text-[9px] text-slate-400 font-medium">
                      {uploadMethod === 'local' ? 'Local Upload' : 'URL Link Asset'}
                    </span>
                  </div>
                  <div className="relative aspect-video w-full border border-slate-200 rounded-xl overflow-hidden bg-slate-50 flex items-center justify-center p-3 shadow-inner group">
              ...
              <div className="space-y-1.5 pt-1">
                <label className="text-[10px] font-bold text-[#475569] uppercase tracking-wider flex items-center gap-1">
                  <Scissors size={12} className="text-indigo-600" /> Background Removal Tool
                </label>
                <button
                  type="button"
                  onClick={handleRemoveBackground}
                  disabled={isRemovingBg || (uploadMethod === 'local' ? !localFilePreview : !newDesign.previewUrl)}
                  className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs rounded-xl border border-indigo-200 shadow-xs transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
                  {isRemovingBg ? (
                    <>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-[#f1f5f9]">
                <button
                  type="button"
                  onClick={handleCloseModal}
                  className="px-4 py-2 rounded-xl text-xs font-semibold border border-slate-200 text-slate-655 hover:bg-slate-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 rounded-xl text-xs font-semibold bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm disabled:opacity-50"
                >
